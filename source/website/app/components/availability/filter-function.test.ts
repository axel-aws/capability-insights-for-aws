...
const ec2 = makeItem({
  id: 'svc-ec2',
  name: 'EC2',
  regionalAvailability: { 'us-east-1': 'Available', 'us-gov-west-1': 'Available', 'us-gov-east-1': 'Not Available' },
});
const instance = makeItem({
  id: 'rt-instance',
  parentId: 'svc-ec2',
  name: 'Instance',
  regionalAvailabilityType: RegionalAvailabilityType.RESOURCE_TYPE,
  regionalAvailability: { 'us-east-1': 'Available', 'us-gov-west-1': 'Available', 'us-gov-east-1': 'Available' },
});
const s3 = makeItem({
  id: 'svc-s3',
  name: 'S3',
  regionalAvailability: { 'us-east-1': 'Available', 'us-gov-west-1': 'Not Available', 'us-gov-east-1': 'Available' },
});
const bucket = makeItem({
  id: 'rt-bucket',
  parentId: 'svc-s3',
  name: 'Bucket',
  regionalAvailabilityType: RegionalAvailabilityType.RESOURCE_TYPE,
...
function groupArb(d: number): fc.Arbitrary<PropertyFilterTokenGroup> {
  if (d <= 1)
    return fc.record({
      ...
      tokens: fc.array(leafArb, { minLength: 0, maxLength: 4 }),
    });
  return fc.record({
    operation: fc.constantFrom('and' as const, 'or' as const),
    tokens: fc.array(fc.oneof({ weight: 3, arbitrary: leafArb }, { weight: 1, arbitrary: groupArb(d - 1) }), {
      minLength: 0,
      maxLength: 4,
    }),
  });
...
function refEval(item: RegionalAvailability, t: PropertyFilterToken | PropertyFilterTokenGroup): boolean {
  if ('operation' in t) {
    const g = t as PropertyFilterTokenGroup;
    return g.operation === 'and'
      ? g.tokens.every(c => refEval(item, c))
      : g.tokens.length > 0 && g.tokens.some(c => refEval(item, c));
  }
  return (
    (item.regionalAvailability?.[(t as PropertyFilterToken).propertyKey!.slice(7)] ?? '') ===
    (t as PropertyFilterToken).value
  );
}

describe('PBT: recursive boolean evaluation', () => {
  it('matches reference evaluator', () => {
    const itemA = fc.record({
      id: fc.constant('t'),
      ...
      regionalAvailability: fc.record({
        'us-east-1': fc.constantFrom(...AVAIL),
        'us-west-2': fc.constantFrom(...AVAIL),
        'eu-west-1': fc.constantFrom(...AVAIL),
      }),
    });
    fc.assert(
      fc.property(groupArb(3), itemA, (group, item) => {
        const filterFn = createFilteringFunction([item]);
        expect(filterFn(item, { operation: group.operation, tokens: [], tokenGroups: group.tokens })).toBe(
          refEval(item, group),
        );
      }),
      { numRuns: 200 },
